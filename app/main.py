...
def getMatchesDataCsv(account_id, query_params={}, api_calls_limit=50, api_limit_duration_seconds=60):
    count = 0
    api_calls_made = 0
    rows = []
    match_fields = ['match_id', 'duration', 'first_blood_time', 'game_mode', 'dire_score', 'radiant_score', 'radiant_win', 'start_time', 'region', 'replay_url']
    ...
    for match in matches_data_json:
        if api_calls_made < api_calls_limit:
            ...
            try:
                ...
                print(f'KeyError occured while getting picks_bans data for match with id {match_id}')
            except TypeError:
                print(f'TypeError occured while getting picks_bans data for match with id {match_id}')
            list_of_picks = []
            try:
                ...
                print(f'KeyError occured while getting data of player with account id {account_id} for match with id {match_id}')
            except TypeError:
                print(f'TypeError occured while getting picks_bans data for match with id {match_id}')
            row.append(list_of_picks)
            rows.append(row)
            print(f'fetched data for match with id: {match_id}')
            # time.sleep(1) # Wait one second after each api call to avoid traffic
        else:
            print('**********************')
